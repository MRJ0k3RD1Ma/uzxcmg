...
function ChunkedFileUploadParallel({ token }) {
      ...
      const uploadWithConcurrency = async () => {
        ...
        const uploadNext = async () => {
          while (currentIndex < chunks.length) {
            const chunkInfo = chunks[currentIndex]
            currentIndex++

            await uploadChunk(chunkInfo.blob, chunkInfo.index, totalChunks, fileId, file.name)

            uploadedChunksRef.current.add(chunkInfo.index)
            const newProgress = Math.round(
              (uploadedChunksRef.current.size / totalChunks) * 100
            )
  ...
  return (
    <div>
      <h2>Parallel Yuklash</h2>
      <p style={{ color: '#666', fontSize: '14px' }}>
        Fayllar bo'laklarga bo'linib bir vaqtda {MAX_CONCURRENT_UPLOADS} ta yuboriladi (tezroq)
      </p>
          ...
          >
            <div
              style={{
                width: `${progress}%`,
                height: '100%',
                backgroundColor: success ? '#4caf50' : '#9c27b0',
...
export default ChunkedFileUploadParallel
